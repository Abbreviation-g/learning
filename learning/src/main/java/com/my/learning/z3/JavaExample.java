...
class JavaExample {
	...
	private BoolExpr commAxiom(Context ctx, FuncDecl f) throws Exception {
		...
		String bench = "(assert (forall (x " + t.getName() + ") (y " + t.getName() + ") (= (" + f.getName() + " x y) ("
				+ f.getName() + " y x))))";
		return ctx.parseSMTLIB2String(bench, new Symbol[] { t.getName() }, new Sort[] { t },
				new Symbol[] { f.getName() }, new FuncDecl[] { f })[0];
	}
	...
	void solveTactical(Context ctx, Tactic t, Goal g, Status sat) throws TestFailedException {
	...
	void sudokuExample(Context ctx) throws TestFailedException {
		...
		for (int i = 0; i < 9; i++)
			for (int j = 0; j < 9; j++)
				if (0 != instance[i][j])
					instance_c = ctx.mkAnd(instance_c, ctx.mkEq(X[i][j], ctx.mkInt(instance[i][j])));
	...
	public void parserExample1(Context ctx) throws TestFailedException {
		...
		BoolExpr f = ctx.parseSMTLIB2String(
				"(declare-const x Int) (declare-const y Int) (assert (and (> x y) (> x 0)))", null, null, null,
				null)[0];
	...
	public void parserExample3(Context ctx) throws Exception {
		...
		BoolExpr thm = ctx.parseSMTLIB2String(
				...
				null, null, new Symbol[] { ctx.mkSymbol("gg") }, new FuncDecl[] { g })[0];
	...
	public void optimizeExample(Context ctx) {
		...
		opt.Add(ctx.mkEq(ctx.mkAdd(xExp, yExp), ctx.mkInt(5)), ctx.mkGe(xExp, ctx.mkInt(1)),
				ctx.mkGe(yExp, ctx.mkInt(2)));
